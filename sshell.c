...
/*
 * Detect if a user command contains double > (>>)
 * append signals
 * @return 1, if there is output append requirement
 */
...
int split_command(char* cmdString, Command* com) {
	char* commandline = malloc(CMDLINE_MAX * sizeof(char));
	...
	while (string != NULL) {
		if (index >= ARGS_MAX) { // Too many arguments
	...
	// Now, all unpopulated fields in com->args should be NULL (used to detect later)
	while (index < ARGS_MAX) {
		com->args[index] = NULL;
		index++;
	}
	free(commandline);
 ...
 * If commandline input is: echo hello world > file1.txt
 * beforeRedirection gets: "echo hello world"
 * afterRedirection gets: "file1.txt"
 */
int split_command_redirection(char* cmdString, Command* com) {

	char* commandline = malloc(CMDLINE_MAX * sizeof(char));
	strcpy(commandline, cmdString);
	char* checkRedirection = malloc(CMDLINE_MAX * sizeof(char));
	...
	if (isRedirection(checkRedirection)) { // Redirection is present
		...
		while (string != NULL) { // While valid string, copy into command arguments
			strcpy(com->args[index], string);
			string = strtok(NULL, spaceDelimiter);
			index++;
		}
...
void split_pipe(char* cmdString, Pipecmd *pipe) {
	...
	char* pipe1Str = malloc(TOKEN_LENGTH_MAX * sizeof(char));
	char* pipe2Str = malloc(TOKEN_LENGTH_MAX * sizeof(char));
	char* pipe3Str = malloc(TOKEN_LENGTH_MAX * sizeof(char));
	char* pipe4Str = malloc(TOKEN_LENGTH_MAX * sizeof(char));
...
int* sshellSystem_pipe(Pipecmd* cmdPipe, int redirectionFlag) {
	// We assume there is up to 3 pipes
	int first_fd[2];
	int second_fd[2];
	int third_fd[2];
...
int sls_command() {
	// Logic referred from Lecture Slides (professor Porquet)
	DIR* dirp;
	struct stat sb;
	struct dirent* dp;
...
int main(void) {
	...
	while (1) {
		...
		com->cmd = malloc(TOKEN_LENGTH_MAX * sizeof(char));
		com->args = malloc(ARGS_MAX * sizeof(char*));
		for (int i = 0; i < ARGS_MAX; i++) {
			com->args[i] = malloc(TOKEN_LENGTH_MAX * sizeof(char));
		}
		com->filename = malloc(TOKEN_LENGTH_MAX * sizeof(char));

		// Pipe structure initializing
		Pipecmd* pipe = NULL;
		pipe = malloc(sizeof(Pipecmd));
		pipe->pipeCommand1 = malloc(sizeof(Command));
		pipe->pipeCommand1->cmd = malloc(TOKEN_LENGTH_MAX * sizeof(char));
		pipe->pipeCommand1->args = malloc(ARGS_MAX * sizeof(char*));
		for (int i = 0; i < ARGS_MAX; i++) {
			pipe->pipeCommand1->args[i] = malloc(TOKEN_LENGTH_MAX * sizeof(char));
		}
		pipe->pipeCommand1->filename = malloc(TOKEN_LENGTH_MAX * sizeof(char));


		pipe->pipeCommand2 = malloc(sizeof(Command));
		pipe->pipeCommand2->cmd = malloc(TOKEN_LENGTH_MAX * sizeof(char));
		pipe->pipeCommand2->args = malloc(ARGS_MAX * sizeof(char*));
		for (int i = 0; i < ARGS_MAX; i++) {
			pipe->pipeCommand2->args[i] = malloc(TOKEN_LENGTH_MAX * sizeof(char));
		}
		pipe->pipeCommand2->filename = malloc(TOKEN_LENGTH_MAX * sizeof(char));

		pipe->pipeCommand3 = malloc(sizeof(Command));
		pipe->pipeCommand3->cmd = malloc(TOKEN_LENGTH_MAX * sizeof(char));
		pipe->pipeCommand3->args = malloc(ARGS_MAX * sizeof(char*));
		for (int i = 0; i < ARGS_MAX; i++) {
			pipe->pipeCommand3->args[i] = malloc(TOKEN_LENGTH_MAX * sizeof(char));
		}
		pipe->pipeCommand3->filename = malloc(TOKEN_LENGTH_MAX * sizeof(char));

		pipe->pipeCommand4 = malloc(sizeof(Command));
		pipe->pipeCommand4->cmd = malloc(TOKEN_LENGTH_MAX * sizeof(char));
		pipe->pipeCommand4->args = malloc(ARGS_MAX * sizeof(char*));
		for (int i = 0; i < ARGS_MAX; i++) {
			pipe->pipeCommand4->args[i] = malloc(TOKEN_LENGTH_MAX * sizeof(char));
		}
		pipe->pipeCommand4->filename = malloc(TOKEN_LENGTH_MAX * sizeof(char));
		...
		strcpy(cmdCopy, cmd); // Reset cmdCopy in case it was modified

		// Append signal detection (>>)
		int appendFlag = 0;
		if (isAppend(cmdCopy)) {
			appendFlag = 1;
		}
		...
		} else if (redirectionFlag == 1 && pipeFlag == 0) { // Redirection but no piping
			int success = split_command_redirection(cmdCopy, com);
			if (success == 1) {
				continue;
			}
		} else if (pipeFlag == 0 && redirectionFlag == 0) { // Plain command (no piping, no redirection)
